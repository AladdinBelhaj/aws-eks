...
resource "aws_eip" "nat_eip" {
  ...
  domain = "vpc"
  tags = {
    Name = "${var.fullname}-${local.env}-eip-${count.index}"
  }
...
resource "aws_nat_gateway" "nat" {
  ...
  tags = {
    Name = "${var.fullname}-${local.env}-nat-gateway-name-${count.index}"
  }
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name = "${var.fullname}-${local.env}-public-rt"
  }
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "${var.fullname}-${local.env}-private-rt-${count.index}"
  }
